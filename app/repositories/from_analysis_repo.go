...
type FaRepo interface {
	...
	FromStatisticAll(db *gorm.DB, param *vo.JSFilterParams, beginAt, endAt string) ([]models.FromAnalysisView,
		exception.Exception)
}
...
func (fri *FaRepoImpl) FromStatisticAll(db *gorm.DB, param *vo.JSFilterParams, beginAt, endAt string) (
	[]models.FromAnalysisView, exception.Exception) {
	data := make([]models.FromAnalysisView, 0)
	subTx := db.Table(tables.IPRecode).Select("js_id, from_url, to_url").
		Where("visit_time >= ? and visit_time <= ?", beginAt, endAt)
	if param.PrimaryID == 0 {
		return nil, exception.New(response.ExceptionInvalidRequestParameters, "primary_id must choose")
	}
	subTx = subTx.Where("primary_id = ?", param.PrimaryID)
	if param.CategoryID != 0 {
		subTx = subTx.Where("category_id = ?", param.CategoryID)
	...
	tx := db.Table("(?) AS sub", subTx).
		...
		Group("js.title, sub.js_id, sub.from_url, sub.to_url").
		Scan(&data)
	return data, exception.Wrap(response.ExceptionDatabase, tx.Error)
}
